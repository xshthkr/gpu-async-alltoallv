 ...
 * parlogna.cpp
 *
 *      From ParLogNa (TuNA) originally implemented by kokofan
 */
    ...
    MPI_Comm comm)
{
    ...
    for (int i { 0 }; i < w - 1; i++) { nlpow *= r; }   // maximum send number of elements
	...
	// optimization check to ensure logarithmic algorithm is used only if its faster than linear alltoall

	if (K < nprocs - 1) {

		// find maximum data-block size max_send_count with MPI_Allreduce
		for (int i { 0 }; i < nprocs; i++) {
			if (sendcounts[i] > local_max_count) { local_max_count = sendcounts[i]; }
		}
		MPI_Allreduce(&local_max_count, &max_send_count, 1, MPI_INT, MPI_MAX, comm);

		// allocate rotation array for each process
		for (int i { 0 }; i < nprocs; i++) { rotate_index_array[i] = (2 * rank - i + nprocs) % nprocs; }

		// allocate temporary buffer with necessary length
	    ...
	    if (extra_buffer == nullptr || temp_recv_buffer == nullptr || temp_send_buffer == nullptr) {
	        std::cerr << "extra_buffer or temp_recv_buffer or temp_send_buffer allocation failed!" << std::endl;
	...
	// copy data that needs to be sent to self
		...
		for (int k { 1 }; k < ze; k += b) {
			...
			for (int s { 0 }; s < ss; s++) {
				...
				if (ns == 1) {
					...
					MPI_Sendrecv(
                        metadata_send, di, MPI_INT, sendrank, 0, 
